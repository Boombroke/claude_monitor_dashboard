...
**在 macOS 上不建议用 Docker 部署本工具。** ccmon 的核心是观察宿主机本地状态——读 `~/.claude/sessions/`、监听文件变化、用 `kill -0 <pid>` 检测进程存活。而 macOS 上 Docker 跑在 Linux VM 里，与宿主隔离：容器看不到 macOS 的进程（会话会全被判成「已结束」）、bind-mount 的文件事件不可靠、`osascript`/`ioreg`（桌面通知/存在感）也用不了。用单文件可执行或源码模式直接在宿主跑才是正解。

> 例外：若监控目标是一台 **Linux 服务器**，Docker 才合理（Linux 原生无 VM）：`docker run --pid=host --network=host -v ~/.claude:/claude:ro ...` 下 `kill -0` 与 inotify 可工作（桌面通知仍无，靠 ntfy）。
...
用 Node SEA（Single Executable Application）把 ccmon 打成一个自包含二进制，**拷到任何同架构 macOS 即可运行，无需安装 Node**：

```bash
# 1. 下载官方 Node 二进制作为注入基底（Homebrew 的 node 不含 SEA fuse，无法用）
mkdir -p .node-official
V=$(node -p 'process.version'); A=$(node -p 'process.arch')
curl -sSL "https://nodejs.org/dist/$V/node-$V-darwin-$A.tar.gz" | tar xz -C .node-official

# 2. 打包（esbuild bundle → SEA blob → postject 注入 → 签名）
npm run build:sea
...
> **权衡说明**：单文件约 **138MB**——因为它把整个 Node 运行时打了进去。它换来的是「零依赖、拷贝即用」的**便携性**，而不是体积上的「轻」。若只在装了 Node 的机器上跑，直接用源码模式（`node src/cli.ts start`）更轻。静态资源（`public/`）会嵌入二进制，运行时自动从内嵌资源服务。

## 开发

```bash
npm run typecheck    # tsc 类型检查
npm test             # node:test 全量单测（37 项）
